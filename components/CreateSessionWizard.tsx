...
import { User, Users, Flame, Cloud, EyeOff, Clock, Hourglass, Shield, Lock, X } from 'lucide-react';
        ...
        <div className="grid grid-cols-2 gap-2">
          {[
            ...
            <button
              ...
              className={`p-2.5 rounded-xl border flex items-center justify-start gap-3 transition-all ${
                config.privacy === p.id 
      ...
      <div className="space-y-3">
        ...
        <div className="grid grid-cols-2 gap-3">
            ...
            <div 
              ...
              className={`p-3 rounded-2xl border cursor-pointer transition-all flex flex-col gap-2 relative overflow-hidden ${
                 config.safety.nsfw ? 'bg-red-50 border-red-200' : 'bg-gray-50 border-gray-100 hover:border-gray-200'
              }`}
            >
               ...
               <div className="text-[10px] text-gray-500 leading-tight z-10">
                 Allow mature themes and NSFW conversations.
               </div>
            ...
            {/* Vibe Shield Toggle - Mutually exclusive with NSFW */}
             <div 
              ...
              className={`p-3 rounded-2xl border cursor-pointer transition-all flex flex-col gap-2 relative overflow-hidden ${
                 config.safety.flirtingProtection ? 'bg-teal-50 border-teal-200' : 'bg-gray-50 border-gray-100 hover:border-gray-200'
              }`}
            >
               <div className="flex justify-between items-start z-10">
            ...
            {/* Slow Mode */}
             <div 
              ...
              className={`p-3 rounded-2xl border cursor-pointer transition-all flex flex-col gap-2 relative overflow-hidden ${
                 config.safety.slowMode ? 'bg-amber-50 border-amber-200' : 'bg-gray-50 border-gray-100 hover:border-gray-200'
              }`}
            >
               <div className="flex justify-between items-start z-10">
             ...
             <div 
              ...
              className={`p-3 rounded-2xl border cursor-pointer transition-all flex flex-col gap-2 relative overflow-hidden ${
                 config.safety.noFiles ? 'bg-indigo-50 border-indigo-200' : 'bg-gray-50 border-gray-100 hover:border-gray-200'
              }`}
            >
               <div className="flex justify-between items-start z-10">
        ...
        <div className="flex gap-2 overflow-x-auto pb-4 scrollbar-hide snap-x justify-center">
          {['💬', '🦊', '🚀', '👻', '💀', '👽', '🤖', '🧠', '💋', '🔥', '🎲', '🎭', '💊', '🔮', '💦', '👁️', '👑', '😈', '🐍', '🕸️', '🦄'].map(e => (
            <button
              ...
              className={`w-14 h-14 flex-shrink-0 rounded-2xl flex items-center justify-center text-3xl border transition-all snap-center ${
                config.emoji === e ? 'bg-blue-50 border-blue-400 scale-110 shadow-md ring-2 ring-blue-100' : 'bg-white border-gray-100 hover:border-gray-300 opacity-60 hover:opacity-100'
              }`}
            >
              {e}
            </button>
